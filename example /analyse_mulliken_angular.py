...
for sp in range(len(s)):
    if sp == 0:
        ...
        #plt.plot(x, s[sp]+p[sp]+d[sp]+f[sp], lw=2, label='f', color=get_graph_colour(3), ls=get_graph_linetype())
    else: # (sp == 1)
        ...
        #plt.plot(x, -(s[sp]+p[sp]+d[sp]+f[sp]), lw=2, color=get_graph_colour(3), ls=get_graph_linetype())
